    ...
    def udp_listen(self):
        ...
        while not rospy.is_shutdown():
            ...
            if data != []:
                ...
                if data_len == 496 and data[0:4] == data[492:496]:
                    dataFormat = f"{120}f" # 30*4 = 120
                    data_flag1 = struct.unpack('i', data[4:8])[0]
                    data_flag2 = struct.unpack('f', data[8:12])[0]
                    data_data = struct.unpack(dataFormat, data[12:492])
                    if data_flag1 == -1:
                        # relative vehicle pose
                        # self.node.get_logger().info('received relative pose')
                        # self.publish_mab_pose_relative(data_data[0:3])
                        pass
                    elif data_flag1 == -2:
                        # map vehicle pose
                        # self.node.get_logger().info('received map pose')
                        self.publish_mab_pose(data_data[0:3])
                    else:
                        # self.node.get_logger().info('received trajectory')
                        # self.publish_mab_trajectory(data_flag1, data_flag2, data_data)
                        pass
                else:
